#!/usr/bin/env python3
"""
Control: Glue development endpoint has job bookmark encryption enabled.

Like CloudWatch Logs encryption, job bookmark encryption for a Glue
development endpoint is not a direct field on the endpoint itself - it
comes from a Glue Security Configuration attached to it, which defines
EncryptionConfiguration.JobBookmarksEncryption.

Checks every Glue development endpoint in every enabled region:
  - No Security Configuration attached at all -> non-compliant.
  - Security Configuration attached -> checked for
    JobBookmarksEncryption.JobBookmarksEncryptionMode == "CSE-KMS" with a
    KMS key configured.
"""
    ...
    for region in tqdm(regions, desc="Scanning Regions"):
        ...
        for endpoint in endpoints:
            ...
            if not security_config_name:
                ...
                non_compliant += 1
                evidence = "No Security Configuration attached - job bookmark encryption is not possible"
                results.append({
                    "Region": region,
            ...
            bookmark_encryption = sc_detail.get("EncryptionConfiguration", {}).get("JobBookmarksEncryption", {})
            bookmark_mode = bookmark_encryption.get("JobBookmarksEncryptionMode", "DISABLED")
            kms_key_arn = bookmark_encryption.get("KmsKeyArn")

            if bookmark_mode == "CSE-KMS" and kms_key_arn:
                status = "COMPLIANT"
                compliant += 1
                evidence = f"Job bookmark encryption is enabled (Security Configuration: {security_config_name}, KMS key: {kms_key_arn})"
            else:
                status = "NON_COMPLIANT"
                non_compliant += 1
                evidence = (
                    f"Job bookmark encryption is not enabled "
                    f"(Security Configuration: {security_config_name}, mode: {bookmark_mode})"
                )
...
def write_csv(results, account_id):
    ...
    filename = f"glue_devendpoint_bookmark_encryption_{account_id}_{timestamp}.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="Check Glue development endpoints have job bookmark encryption enabled."
    )
    ...
    control_name = "Glue - Development Endpoint Job Bookmark Encryption Enabled"
