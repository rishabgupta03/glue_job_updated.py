#!/usr/bin/env python3
"""
Control     : Glue job has S3 encryption enabled
Description : For every enabled region, checks each AWS Glue job's attached
              ...
              Security Configuration attached AND that configuration's S3
              encryption mode is SSE-S3 or SSE-KMS (not DISABLED, and not
              missing). Security configurations are cached per region since
              many jobs typically share the same one - avoids a redundant
              GetSecurityConfiguration call per job.
"""
...
CONTROL_NAME = "Glue Job Has S3 Encryption Enabled"
...
def get_s3_encryption_status(glue_client, config_name: str, cache: dict):
    """
    Returns (is_encrypted: bool, detail: str) for a named Security Configuration.
    Cached per region - many Glue jobs share the same configuration.
    """
    ...
    s3_encryptions = (
        response.get("SecurityConfiguration", {})
        .get("EncryptionConfiguration", {})
        .get("S3Encryption", [])
    )
    modes = {e.get("S3EncryptionMode", "DISABLED") for e in s3_encryptions}
    enabled_modes = modes - {"DISABLED"}

    if enabled_modes:
        result = (True, f"S3 encryption mode: {', '.join(sorted(enabled_modes))}")
    else:
        result = (False, "Security configuration exists but S3 encryption mode is DISABLED")
...
def check_control(session: Session):
    ...
    for region in tqdm(regions, desc="Scanning regions"):
        ...
        for job in jobs:
            ...
            try:
                is_encrypted, detail = get_s3_encryption_status(glue, config_name, security_config_cache)
                total_checked += 1
                if is_encrypted:
                    compliant += 1
...
def write_csv(results, account_id):
    filename = f"glue_job_s3_encryption_enabled_{account_id}.csv"
