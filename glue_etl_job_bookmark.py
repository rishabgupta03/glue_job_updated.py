#!/usr/bin/env python3
"""
Control     : Glue ETL job has job bookmark encryption enabled
Description : For every enabled region, checks each AWS Glue job's attached
              ...
              Security Configuration attached AND that configuration's job
              bookmark encryption mode is CSE-KMS (not DISABLED, and not
              missing). Security configurations are cached per region as a
              generic EncryptionConfiguration dict - many jobs share the
              same one, and caching the whole dict (rather than just the
              bookmarks field) means the same cache can answer S3 or
              CloudWatch encryption questions too without extra API calls.
"""
...
CONTROL_NAME = "Glue ETL Job Has Job Bookmark Encryption Enabled"
...
def bookmark_encryption_status(encryption_config: dict):
    """Returns (is_encrypted: bool, detail: str) for the job bookmark encryption field."""
    mode = encryption_config.get("JobBookmarksEncryption", {}).get("JobBookmarksEncryptionMode", "DISABLED")
    if mode != "DISABLED":
        return True, f"Job bookmark encryption mode: {mode}"
    return False, "Security configuration exists but job bookmark encryption mode is DISABLED"
    ...
    for region in tqdm(regions, desc="Scanning regions"):
        ...
        for job in jobs:
            ...
            try:
                ...
                is_encrypted, detail = bookmark_encryption_status(encryption_config)
                total_checked += 1
                if is_encrypted:
                    compliant += 1
...
def write_csv(results, account_id):
    filename = f"glue_etl_job_bookmark_encryption_enabled_{account_id}.csv"
